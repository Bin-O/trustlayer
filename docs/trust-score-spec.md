...
## 8-C. 多在留資格対応の設計方針(Phase C)

現行スコアは特定技能1号を前提に義務データ(面談・定期届出等)を分子・分母に置く。
技人国・技能実習・永住等へ拡張する際、義務体系が資格ごとに異なるため、
「義務が無い項目で減点される」歪みを避ける三方針を採る。

1. **法定データの任意HR実践化(データ形式は不変)**
   面談・支援記録など特定技能の法定業務を、非特定技能資格では「任意実施の
   HR実践」として同一データ形式で提供する。テーブル・入力UIは共通のまま、
   義務か任意かはメタ情報で区別する(記録の再利用性を保つ)。

2. **在留資格ごとの分母正規化**
   スコアの分母を、その資格で利用可能なデータ源に応じて正規化する。
   - 義務が無い項目は分母から除外(未実施でも減点しない)
   - 義務があるのに記録ゼロの項目のみ減点する
   これにより資格をまたいでスコアを公平に比較できる。

3. **資格中立データ源への重心移行(長期)**
   最終的には在留資格に依存しないデータ源 — 賃金台帳・社会保険加入・
   在留期間更新の連続性 — に評価の重心を移す。資格固有の義務データは
   加点材料に留め、中立シグナルを土台に据える。
