...
**確認の求め方**: WBへの確認・質問・裁定依頼を行う際は、DB変更等のリスク
操作に限らず**すべての場合**において、技術的な内容の後に中国語で要点を添える。
中国語に含める内容:
- ① 何を聞いているか
- ② 選択肢がある場合はその違い
- ③ リスクの有無
WBが日本語の技術文を読まなくても判断できる形にする。

**権限確認ダイアログが出そうなコマンドの直前**: システムの権限確認ダイアログが
出そうなコマンド(複合bash・リダイレクト含む等)を実行する直前に、そのコマンドが
何をするか+読取専用か否かを**中国語一句で先に述べる**。ダイアログ自体には中文を
入れられないため、直前のメッセージで補う。
