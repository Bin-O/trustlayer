...
### 承認運用(2026-07-12 改定)
**確認不要・自動実行してよい(実行後にまとめて報告)**:
- 読取系(SELECT / cat / grep / ls)
- テスト(Playwright / tsc / npm)、ビルド、devサーバー
- 一時検証スクリプト(node -e 含む)、ファイルコピー
- 承認済み方針内のコード編集

**実行前に内容を提示して明示承認を待つ**:
- DELETE / UPDATE / TRUNCATE の実行(先に対象行を SELECT で提示)
- CREATE / ALTER / DROP TABLE 等のDBスキーマ変更(SQL案を提示)
- git push(まとめての push も都度確認)
- 認証情報・env ファイルの変更

**確認の求め方**: 技術内容の後に必ず「一句話でこれが何か+リスク有無」を
中国語で添える(ユーザーが判断できる形で提示する)
